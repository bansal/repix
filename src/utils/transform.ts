...
import type { Colors, TransformParams, TransformResult } from "../types";
...
export async function transformImage(
  ...
  try {
    ...
    if (!metadata.width || !metadata.height) {
      throw RepixErrors.invalidImage("Could not read image metadata");
    }
    ...
    switch (format) {
      ...
      default:
        throw RepixErrors.unsupportedFormat(format);
    }
    ...
    if (
      ...
      throw RepixErrors.invalidImage("Unsupported image format");
    }

    if (error.message.includes("Image too large")) {
      throw RepixErrors.imageTooLarge();
    }
 ...
 */
function parseColor(color: string): Colors[string] {
  // Handle hex colors
