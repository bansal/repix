...
describe("parseTransformParams", () => {
  ...
  describe("custom transform parameters", () => {
    ...
    it("parses with param aliases (w, h, q, f, g)", () => {
      const result = parseTransformParams(
        "w=500,h=400,q=90,f=webp,g=center",
        {},
        true,
      );
      expect(result).toEqual({
    ...
    it("parses fit options", () => {
      ...
      const scaleDown = parseTransformParams(
        "w=400,h=400,fit=scale-down",
        {},
        true,
      );
  ...
  describe("allowCustomTransforms", () => {
    ...
    it("throws when custom transforms disabled and not a preset", () => {
      expect(() => parseTransformParams("w=100,h=100", presets, false)).toThrow(
        "Custom transformations not allowed",
      );
    });
